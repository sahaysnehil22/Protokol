...
export function createApiRouter(db) {
      ...
      `).run(id, name, contract_number, entity, execution_mode, location || '', road_section || '', timezone || 'America/Lima', timezone_offset || '-05:00', whatsapp_recipients || '', sampling_basis || 'PER_TRUCK', cylinders_per_truck ? parseInt(String(cylinders_per_truck), 10) : 4, default_design_fc ? parseFloat(String(default_design_fc)) : 210);
        ...
        `);
                for (const c of criteria) {
                    ...
                    insertCrit.run(critId, id, c.activity, c.field, c.operator, c.min_value !== undefined ? c.min_value : null, c.max_value !== undefined ? c.max_value : null, c.allowed_values ? (typeof c.allowed_values === 'string' ? c.allowed_values : JSON.stringify(c.allowed_values)) : null, c.expected_value || null, c.unit || null, c.source_reference || 'Expediente Técnico', c.hold_point ? 1 : 0, c.is_active !== undefined ? c.is_active : 1);
                }
            }
        ...
        `);
                for (const t of technicians) {
                    ...
                    insertTech.run(techId, id, t.name, pinHash, deviceToken, t.whatsapp || '', t.role || 'Quality Specialist', t.cip_number || null);
                }
            }
    ...
    router.patch('/projects/:id', (req, res) => {
        try {
            ...
            const { name, contract_number, entity, execution_mode, location, road_section, timezone, timezone_offset, whatsapp_recipients, sampling_basis, cylinders_per_truck, default_design_fc } = req.body;
            db.prepare(`
        UPDATE projects
        ...
        WHERE id = ?
      `).run(name !== undefined ? name : existing.name, contract_number !== undefined ? contract_number : existing.contract_number, entity !== undefined ? entity : existing.entity, execution_mode !== undefined ? execution_mode : existing.execution_mode, location !== undefined ? location : existing.location, road_section !== undefined ? road_section : existing.road_section, timezone !== undefined ? timezone : existing.timezone, timezone_offset !== undefined ? timezone_offset : existing.timezone_offset, whatsapp_recipients !== undefined ? whatsapp_recipients : existing.whatsapp_recipients, sampling_basis !== undefined ? sampling_basis : existing.sampling_basis, cylinders_per_truck !== undefined ? parseInt(String(cylinders_per_truck), 10) : existing.cylinders_per_truck, default_design_fc !== undefined ? parseFloat(String(default_design_fc)) : existing.default_design_fc, projectId);
