export class ValidationService {
     ...
     */
    getCriteria(projectId, activity) {
    ...
    `);
        const rows = stmt.all(projectId, activity);
        return rows.map(r => {
                ...
                if (Array.isArray(r.allowed_values)) {
                    ...
                    try {
                        ...
                        if (Array.isArray(parsed))
                            allowedValues = parsed.map(String);
                    }
                    catch {
                        allowedValues = r.allowed_values.split(',').map((s) => s.trim());
                    }
                }
            }
            return {
     ...
     */
    validateMeasurements(projectId, activity, measurements) {
        ...
        if (activity === 'CONCRETE' && Array.isArray(measurements.trucks) && measurements.trucks.length > 0) {
            ...
            const trucks = measurements.trucks;
     ...
     */
    evaluateCriterion(crit, actualValue) {
