...
export class SupabaseSyncService {
     ...
     */
    static async syncFromSupabase(db) {
        ...
        `);
                for (const p of projects) {
                    insertProj.run(p.id, p.name, p.contract_number, p.entity, p.execution_mode, p.location || null, p.road_section || null, p.timezone || 'America/Lima', p.timezone_offset || '-05:00', p.whatsapp_recipients || null, p.sampling_basis || 'PER_TRUCK', p.cylinders_per_truck || 4, p.default_design_fc || 210, p.created_at || new Date().toISOString());
                }
            }
            // 2. Sync Technicians
        ...
        `);
                for (const t of technicians) {
                    insertTech.run(t.id, t.project_id, t.name, t.pin_hash, t.device_token, t.whatsapp || '', t.role, t.cip_number || null, t.created_at || new Date().toISOString());
                }
            }
            // 3. Sync Criteria
        ...
        `);
                for (const c of criteria) {
                    insertCrit.run(c.id, c.project_id, c.activity, c.field, c.operator, c.min_value !== undefined ? c.min_value : null, c.max_value !== undefined ? c.max_value : null, c.expected_value || null, c.unit || null, c.source_reference, c.is_active !== undefined ? c.is_active : 1, c.created_at || new Date().toISOString());
                }
            }
            // 4. Sync Protocols
        ...
        `);
                for (const tr of trucks) {
                    insertTruck.run(tr.id, tr.protocol_id, tr.truck_number, tr.mixer_id, tr.delivery_note, tr.slump_cm, tr.cylinders_cast, tr.design_fc, tr.slump_verdict, tr.notes || null, tr.created_at || new Date().toISOString());
                }
            }
            // 6. Sync Cylinders
        ...
        `);
                for (const cyl of cylinders) {
                    insertCyl.run(cyl.id, cyl.protocol_id, cyl.truck_id || null, cyl.truck_number || null, cyl.specimen_number || null, cyl.cylinder_code, cyl.cast_date, cyl.test_date || null, cyl.age_days, cyl.strength_kgcm2 !== undefined ? cyl.strength_kgcm2 : null, cyl.design_fc, cyl.lab || null, cyl.report_photo_id || null, cyl.status, cyl.verdict || null, cyl.created_at || new Date().toISOString());
                }
            }
     ...
     */
    static async pushProject(project, criteria = [], technicians = []) {
        if (!isSupabaseConfigured())
            return;
        const supabase = getSupabaseClient();
        if (!supabase)
     ...
     */
    static async pushProtocol(protocol, trucks = [], cylinders = [], nonconformances = []) {
        if (!isSupabaseConfigured())
            return;
        const supabase = getSupabaseClient();
        if (!supabase)
     ...
     */
    static async pushCylinderResult(cylinder, newProtocolVerdict, nc = null) {
        if (!isSupabaseConfigured())
            return;
        const supabase = getSupabaseClient();
        if (!supabase)
