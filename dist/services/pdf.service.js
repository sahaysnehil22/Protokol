    ...
    `).all(params.protocol.id);
      ...
      `).all(params.protocol.id);
            if (protocolChecks.length > 0) {
                ...
                doc.fillColor('#0F172A').fontSize(9).font('Helvetica-Bold').text(lang === 'en' ? 'PROTOCOL INSPECTION CHECKLIST (CUMPLE / NO CUMPLE / NO APLICA)' : 'LISTA DE CHEQUEO DEL PROTOCOLO (CUMPLE / NO CUMPLE / NO APLICA)', 40, y);
                y += 14;
                doc.rect(40, y, 515, 18).fill('#E2E8F0');
                doc.fillColor('#1E293B').fontSize(7.5).font('Helvetica-Bold');
                doc.text('#', 45, y + 5);
      ...
      `).all(params.protocol.id);
            ...
            doc.fillColor('#0F172A').fontSize(9).font('Helvetica-Bold').text(lang === 'en' ? 'OFFICIAL SIGNATURES & STAMPS (RESPONSIBLE STAFF)' : 'CUADRO DE FIRMAS Y SELLOS OFICIALES DE CONFORMIDAD', 40, y);
            y += 14;
            ...
            for (let i = 0; i < sigList.length; i++) {
                ...
                doc.fillColor('#F8FAFC').fontSize(5.2).font('Helvetica-Bold').text(s.role, bx + 2, y + 3, { width: boxWidth - 4, align: 'center' });
                ...
                // Signatory Name & CIP
                doc.fillColor('#0F172A').fontSize(5.5).font('Helvetica-Bold').text(s.signatory_name || 'Ingeniero Responsable', bx + 2, y + 49, { width: boxWidth - 4, align: 'center' });
                doc.fillColor('#475569').fontSize(5).font('Helvetica').text(s.cip_number ? `CIP N° ${s.cip_number}` : 'CONTRATISTA', bx + 2, y + 61, { width: boxWidth - 4, align: 'center' });
            }
            y += boxHeight + 12;
            ...
            doc.text(`Generado automáticamente de conformidad con Directiva N° 017-2023-CG/GMPL e INFOBRAS. Registro inmutable.`, 48, y + 16, { width: 500 });
            doc.end();
