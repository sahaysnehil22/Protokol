...
export class ProtocolService {
     ...
     */
    async submitProtocol(request, lang = 'es') {
    ...
    `).get(request.project_id, request.device_token, request.device_token);
      ...
      `).all(request.project_id);
        ...
        if (request.activity === 'CONCRETE') {
            ...
            if (incomingTrucks && incomingTrucks.length > 0) {
                concreteTrucks = incomingTrucks.map((t, idx) => {
        ...
        const valResult = this.validationService.validateMeasurements(request.project_id, request.activity, measurements);
    ...
    `);
        insertProtocol.run(protocolId, request.project_id, request.activity, recordedAt, serverReceivedAt, timezoneOffset, request.gps.lat, request.gps.lng, request.panel, request.chainage, JSON.stringify(measurements), verdict, verifiedTech.id, request.device_token, integrityHash, `${protocolId}.pdf`, null, request.notes || '', request.idempotency_key || null);
        // 11. Concrete Normalized Relational Storage (Trucks & Cylinders)
        const pending = [];
        const truckRecordMap = new Map(); // truck_number -> truck_id
        if (request.activity === 'CONCRETE') {
      ...
      `);
            for (const truck of concreteTrucks) {
                ...
                insertTruck.run(truckId, protocolId, truck.truck_number, truck.mixer_id, truck.delivery_note, truck.slump || '4', truck.slump_cm || (parseFloat(truck.slump || '4') * 2.54), truck.supplier || 'Concreto Titán', truckCylinders, truckDesignFc, isSlumpPass ? 'PASS' : 'FAIL', truck.notes || '');
                ...
                for (let c = 1; c <= truckCylinders; c++) {
                    ...
                    insertCyl.run(cylId, protocolId, truckId, truck.truck_number, c, cylCode, castDate, ageDays, truckDesignFc);
                }
            }
    ...
    `);
      ...
      `).all(request.project_id, request.activity);
    ...
    `).all(request.project_id);
        ...
        for (const r of defaultRoster) {
            const matchedTech = projectTechs.find(t => (r.sign_order === 1 && (t.role.toLowerCase().includes('quality') || t.name.toLowerCase().includes('david'))) ||
            ...
            insertSig.run(sigId, protocolId, techId || verifiedTech.id, techName, r.role, r.sign_order, techCip, status, signedAt, sigKey, stampKey);
        }
        ...
        `);
                insertNc.run(ncId, protocolId, linkedTruckId, failedCheck.field, failedCheck.expected, String(failedCheck.actual), `Criterio no cumplido en ${failedCheck.field}: obtenido ${failedCheck.actual}, requerido ${failedCheck.expected}`);
     ...
     */
    async recordCylinderResult(protocolId, result, lang = 'es') {
    ...
    `).get(protocolId, result.cylinder_code);
        if (!cylinder) {
        ...
        LIMIT 1
      `).get(protocolId, result.age_days);
        }
        if (!cylinder) {
      ...
      WHERE id = ?
    `).run(result.strength_kgcm2, result.lab, result.report_photo_id || null, testDate, passed ? 'PASS' : 'FAIL', cylinder.id);
        let newProtocolVerdict = protocol.verdict;
        let nonconformanceId = null;
        ...
        `).run(nonconformanceId, protocolId, cylinder.truck_id || null, `>= ${designFc} kg/cm²`, `${result.strength_kgcm2} kg/cm²`, `Resistencia a 28 días (${result.strength_kgcm2} kg/cm²) inferior a f'c (${designFc} kg/cm²) para probeta ${cylinder.cylinder_code}`);
                this.db.prepare(`
        ...
        const tech = this.db.prepare(`SELECT name, role FROM technicians WHERE id = ?`).get(protocol.technician_id);
    ...
    `).all(projectId);
        ...
        const protocolSummaries = protocols.map(p => {
            if (p.verdict === 'PASS')
                passedCount++;
            else if (p.verdict === 'PROVISIONAL_PASS')
                provisionalCount++;
            else if (p.verdict === 'FAIL')
                failedCount++;
     ...
     */
    closeNonConformance(ncId, correctiveAction, technicianId) {
     ...
     */
    buildSubmissionResponse(protocol) {
     ...
     */
    getProtocolSignatures(protocolId) {
     ...
     */
    getChecklistTemplates(projectId, activity) {
        if (activity) {
            return this.db.prepare(`
        SELECT * FROM checklist_templates
        WHERE project_id = ? AND activity = ? AND active = 1
        ORDER BY item_order ASC
      `).all(projectId, activity);
        }
        return this.db.prepare(`
      SELECT * FROM checklist_templates
     ...
     */
    async signProtocol(protocolId, signRequest) {
        const protocol = this.getProtocolById(protocolId);
        if (!protocol) {
            throw new Error(`PROTOCOL_NOT_FOUND: Protocolo ${protocolId} no encontrado.`);
        }
    ...
    `).get(protocolId, tech.id, tech.name);
        if (!sigRecord) {
      ...
      `).get(protocolId);
        ...
        WHERE id = ?
      `).run(tech.id, tech.name, tech.cip_number || sigRecord.cip_number, now, sigKey, stampKey, sigRecord.id);
        }
        else {
      ...
      `).run(newSigId, protocolId, tech.id, tech.name, tech.role, 1, tech.cip_number || null, now, sigKey, stampKey);
        }
